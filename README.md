...
4. **colgroup 列宽 + 全宽展示** — `<colgroup><col width="N" />` 可通过 API 直接设定列宽比例（建议≤750px；超出不剥离但标准宽度下容器裁切，开全宽展示可正常显示）；全宽展示与自适应宽度**独立**，可直接点击工具栏第一个无名 icon 开启。操作顺序：colgroup 设比例 → 标宽下拖拽精调 → 全宽展示
...
2. 修正 colgroup 750px 限制：超 750px 不会被 API 剥离，只是标准宽度下容器裁切溢出部分，开全宽展示可正常显示
3. 修正全宽展示与自适应宽度的关系：两者**独立**，全宽展示可直接点击工具栏第一个无名 icon，无需先开自适应宽度
4. 明确列宽操作顺序：先 colgroup 设比例，再标宽下拖拽精调，最后全宽展示
5. 新增表格工具栏 5 个概念详解（自适应宽度、列等宽、全宽/标宽展示、全屏、拖拽手柄）
